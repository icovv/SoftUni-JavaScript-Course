...
function isUser(user:unknown): user is User{
    ...
    && "passwordHash" in user && ((typeof user.passwordHash == "string" && user.passwordHash.length === 20) || (Array.isArray(user.passwordHash) && user.passwordHash.length == 4 && user.passwordHash.every(x => typeof x == "string" && x.length === 8)))
