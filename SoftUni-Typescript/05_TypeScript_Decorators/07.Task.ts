...
function getLatest(seconds: number) {
    return function (target: any, methodName: string, descriptor: PropertyDescriptor) {
        ...
        descriptor.value = function () {
            let allItems = originalMethod.call(this);
            let currTime = new Date();
            let recentItems = allItems.filter((x: any) => currTime.getTime() - x.createdOn.getTime() <= seconds * 1000);
            return recentItems;
        }
...
function censorProps<T extends { [key: string]: any }>(censorService: MockCensorService<T>) {
    return function (target: any, methodName: string, descriptor: PropertyDescriptor) {
        ...
        descriptor.value = function () {
            let allItems = originalMethod.call(this);
            let censoredItems = censorService.censorProperties(allItems);
            return censoredItems;
        }
        return descriptor;
    }
...
function log(target: any, methodName: string, descriptor: PropertyDescriptor) {
    ...
    descriptor.value = function () {
        console.log(`Method ${methodName} called successfull`)
...
setTimeout(() => {
    ...
    usersService.addUser(user3); usersService.addEmployee(emp3);
